...
function exportarPDF(){
  ...
  const body=ventasActuales.map((r,i)=>[
    String(i+1),texto(r.cliente),texto(r.rucCed),formatoFecha(r.fecha),texto(r.factura),texto(r.autorizacion),money(r.subtotalSinIva),money(r.subtotalConIva),money(r.iva),money(r.total),money(r.retIva),money(r.retRenta),texto(r.numRetencion)
  ]);
  ...
  doc.autoTable({
    ...
    didParseCell:(data)=>{if(data.section==='body'&&data.column.index===12&&data.cell.raw===null)data.cell.text=['SIN RETENCION']},
