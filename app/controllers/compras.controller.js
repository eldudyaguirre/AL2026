...
async function comprasTest(req, res) {
  ...
  console.log('[COMPRAS-TEST] INICIO NOMBRE', { inicio, fin });
  try {
    const result = await pool.query({
      text: `SELECT c.numfaccom, p.ruccedpro, p.nomprovee FROM compras c LEFT JOIN proveedores p ON p.ruccedpro = c.ruccedpro WHERE c.feccompra >= $1::date AND c.feccompra <= $2::date`,
      values: [inicio, fin],
      rowMode: 'array',
    });
    console.log('[COMPRAS-TEST] QUERY NOMBRE TERMINADA', { filas: result.rows.length, tiempoMs: Date.now() - inicioConsulta });
    return res.json({ inicio, fin, tiempoMs: Date.now() - inicioConsulta, total: result.rows.length, filas: result.rows });
  } catch (error) {
    console.error('[COMPRAS-TEST] ERROR NOMBRE', { mensaje: error.message, codigo: error.code, tiempoMs: Date.now() - inicioConsulta });
