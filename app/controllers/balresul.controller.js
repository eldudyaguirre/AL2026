...
async function leerCuentas(client, tabla, limite, mes, anio, historica) {
  ...
  const requeridas = ['codcuenta', 'nomcuenta', 'salfinper', 'saliniper'];
  ...
  const filtros = [
    ...
    `cast(substring(trim(cp.${c('codcuenta')}::text),1,1) as integer) >= 4`,
    `cast(substring(trim(cp.${c('codcuenta')}::text),1,1) as integer) <= 5`,
  ...
  const result = await client.query(`
    SELECT
      ...
      COALESCE(cp.${c('saliniper')},0)::numeric AS "salIniPer"
    FROM ${tablaSQL} cp
...
function clasificar(rows) {
  ...
  for (const row of rows) {
    const salFinPer = Number(row.salFinPer) || 0;
    const salIniPer = Number(row.salIniPer) || 0;
    const cuenta = {
      codCuenta: row.codCuenta,
      nomCuenta: row.nomCuenta || '',
      salFinPer,
      valorPeriodo: salFinPer - salIniPer,
      nivel: Math.max(0, Math.ceil(String(row.codCuenta).length / 2) - 1)
    };
    const tipo = String(row.codCuenta).charAt(0);
    if (tipo === '4') ingresos.push(cuenta);
    else if (tipo === '5') gastos.push(cuenta);
  }

  const total = lista => lista
    .filter(x => String(x.codCuenta).length === 1)
    .reduce((s, x) => s + x.salFinPer, 0);

  const totalPeriodo = lista => lista
    .filter(x => String(x.codCuenta).length === 1)
    .reduce((s, x) => s + x.valorPeriodo, 0);

  const totalIngresos = total(ingresos);
  const totalGastos = total(gastos);
  const totalIngresosPeriodo = totalPeriodo(ingresos);
  const totalGastosPeriodo = totalPeriodo(gastos);
  ...
  return {
    ...
    resultadoPeriodo: totalIngresosPeriodo - totalGastosPeriodo
  };
}

async function balanceResultados(req, res) {
  ...
  try {
    ...
    console.error('[BALRESUL] Error:', error);
    return res.status(500).json({ error: 'Error consultando Balance de Resultados.', detail: error.message, codigo: error.code });
  } finally {
    if (client) {
...
module.exports = { balanceResultados };
